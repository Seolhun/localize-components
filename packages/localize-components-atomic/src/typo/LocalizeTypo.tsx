...
const getStyleLocalizeTypo = (type: LocalizeTypoType) => {
  ...
  if (type === 'big' || type === 'medium' || type === 'small') {
    return styled.span<LocalizeTypoProps, ILocalizeTheme>(
      ({ theme, weight, ...props }) => {
  ...
  return styled[type]<LocalizeTypoProps, ILocalizeTheme>(
    ({ theme, weight, isHighlight, ...props }) => {
      const { highlightedFontColor, primaryFontColor } = getValidThemeObject(
        props,
        theme,
      );
      return {
...
const LocalizeTypo: React.FC<LocalizeTypoProps> = ({
  type,
  children,
  weight = 400,
  css = {},
...
export { LocalizeTypoProps, LocalizeTypo };
